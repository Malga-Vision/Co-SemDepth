...
class DataLoaderGeneric():
    ...
    def _augmentation_step_flip(self):
        ...
        def do_nothing():
            ...
            return [im_col, im_semantic, rot, trans, c]
        ...
        def true_flip_v():
            ...
            return [col, sem, r, t, c_]

        p_order = tf.random.uniform(shape=[], minval=0., maxval=1., dtype=tf.float32)
        pred = tf.less(p_order, 0.5)
        #im_col, im_depth, rot, trans, c = tf.cond(pred, true_flip_v, do_nothing)
        im_col, im_semantic, rot, trans, c = tf.cond(pred, true_flip_v, do_nothing)
        ...
        im_col, im_semantic, rot, trans, c = tf.cond(pred, true_flip_h, do_nothing)
