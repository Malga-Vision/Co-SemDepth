...
class DataLoaderTopAir(DataLoaderGeneric):
    """Dataloader for the TopAir dataset
    """
    def __init__(self, out_size=[384,384], crop=False):
        super(DataLoaderTopAir, self).__init__('topair')

        self.in_size = [384, 384]
        self.depth_type = "map"
        self.class_count = 9
    ...
    def get_dataset(self, usecase, settings, batch_size=3, out_size=[384, 384], crop=False):
        ...
        super(DataLoaderTopAir, self).get_dataset(usecase, settings, batch_size=batch_size, out_size=out_size)
    ...
    def _decode_samples(self, data_sample):
        ...
        if 'depth' in data_sample:
            ...
            image = tf.image.decode_png(file, channels=1,dtype=tf.uint8)
            image = tf.cast(image, dtype=tf.float32)
            depth = image *(100.0/255.0)
            out_data['depth'] = tf.reshape(tf.image.resize(depth, self.intermediate_size, method='nearest'), self.intermediate_size + [1])
