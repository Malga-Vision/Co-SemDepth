"""
----------------------------------------------------------------------------------------
Copyright (c) 2025 - Yara AlaaEldin, University of Genova, Italy.
...
class CustomMidairValidationCallback(ks.callbacks.Callback):
    ''' Custom callbacks designed to launch validation on the Midair dataset after each epoch'''
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        print(save_path + main_command + options + opt_args + "> /home/yara/")
        os.system(main_command + options + opt_args)
    ...
    def update_backup(self, perfs):
        ...
        # else find if current candidate is better than any existing saved backup
            ...
            for i in range(best_perfs.shape[0]):
                ...
                metrics = ["miou"]
                for metric in metrics:
                    mnter = mnter + 1 if best_perfs[metric].iloc[i] < perfs[metric][0] else mnter
                # If a majority of metrics are better, the list of best performing models is updated
                if cnter > 3 and mnter > 0:
                    perfs["ckpt_name"] = self.backup_last_ckpt()
                    df = pd.from_dict(perfs)
                    best_perfs = best_perfs.append(df, ignore_index=True)
                    print(best_perfs)
