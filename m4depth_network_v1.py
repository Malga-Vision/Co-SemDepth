...
class SemanticEstimatorLevel(ks.layers.Layer):
    ...
    def build(self, input_shapes):
        ...
        if (not self.is_training):
            ...
            self.semantic_prev_t = self.add_weight(name="semantic_prev_t", shape=self.shape[:3] + [self.classes], dtype='float32',
                                                initializer=s_maps_init, trainable=False, use_resource=False)
        else:
            print("Skipping temporal memory instanciation")

    @tf.function
    def call(self, curr_f_maps, prev_l_est, rot, trans, camera, new_traj, prev_f_maps=None, prev_t_semantic=None):
        with tf.name_scope("SemanticEstimator_lvl"):
            ...
            if (not self.is_training) and prev_f_maps is None and prev_t_semantic is None:
                prev_t_semantic = self.semantic_prev_t
                prev_f_maps = self.prev_f_maps

            if prev_l_est is None:
                # Initial state of variables
                semantic_prev_l = tf.zeros([b, h, w, self.classes-1])
                other_class = tf.ones([b, h, w, 1])
                semantic_prev_l = tf.concat([semantic_prev_l, other_class], axis = 3)
                #semantic_prev_l[:,:,:,13] = 1.0 ## initialize the probability of the last class to 1 -> this corresponds to 'other' class
                other_prev_l = tf.zeros([b, h, w, 4])
            else:
                semantic_prev_l = tf.compat.v1.image.resize(prev_l_est["semantic"], [h, w], method = "nearest")
                other_prev_l = tf.compat.v1.image.resize(prev_l_est["other"], [h, w], method = "nearest")
            ...
            if prev_t_semantic is None or new_traj[0]:
                prev_t_semantic = tf.zeros(self.shape[:3] + [self.classes-1], dtype='float32')
                other_class = tf.ones(self.shape[:3] + [1], dtype='float32')
                prev_t_semantic = tf.concat([prev_t_semantic, other_class], axis = 3)
                #prev_t_semantic[:,:,:,13] = 1.0
                if not self.is_training:
                    self.prev_f_maps.assign(curr_f_maps)
                    self.semantic_prev_t.assign(prev_t_semantic)
                    
                curr_l_est = {"semantic": semantic_prev_l, "other": other_prev_l}
                return curr_l_est
            else:
                with tf.name_scope("preprocessor"):
                    ...
                    with tf.name_scope("input_prep"):
                        ...
                        input_features = [semantic_prev_l]
                ...
                with tf.name_scope("semantic_estimator"):
                    ...
                    if not self.is_training:
                        self.prev_f_maps.assign(curr_f_maps)
                        self.semantic_prev_t.assign(semantic)
            ...
            with tf.name_scope("SemanticEstimator_seq"):
                ...
                for l, (f_maps_curr, level) in enumerate(zip(f_pyr_curr[::-1], self.levels[::-1])):
                    ...
                    if self.is_training and seq_i != 0:
                        ...
                        s_est_prev = s_est_seq[-1][-l - 1]["semantic"]
                    ...
                    if s_est_curr is None:
                        s_est_curr = [level(f_maps_curr, None, local_rot, local_trans, local_camera, new_traj,
                                            prev_f_maps=f_maps_prev, prev_t_semantic=s_est_prev)]
                    else:
                        s_est_curr.append(
                            level(f_maps_curr, s_est, local_rot, local_trans, local_camera, new_traj,
                                  prev_f_maps=f_maps_prev, prev_t_semantic=s_est_prev))
                    cnter -= 1.

                s_est_seq.append(s_est_curr[::-1])
        return s_est_seq
